...
define("XFRAMEWORK_VERSION", "1.7.6");
...
abstract class Plugin extends \rcube_plugin
{
    ...
    public function updateDatabase()
    {
        ...
        if (!isset($this->rcmail->xversions)) {
            if ($result = $this->db->value("value", "system", ["name" => "xframework_db_versions"])) {
                $this->rcmail->xversions = json_decode($result, true);
                $this->rcmail->xversionsUpdate = true;
            } else {
                $this->rcmail->xversions = [];
                $this->rcmail->xversionsUpdate = false;
            }
        }

        // get the schema version of this plugin that exists in the database
        $existingVersion = array_key_exists($this->plugin, $this->rcmail->xversions) ? $this->rcmail->xversions[$this->plugin] : 0;
        ...
        $this->rcmail->xversions[$this->plugin] = $this->databaseVersion;

        if ($this->rcmail->xversionsUpdate) {
            $this->db->update("system", ["value" => json_encode($this->rcmail->xversions)], ["name" => "xframework_db_versions"]);
        } else {
            $this->db->insert("system", ["name" => "xframework_db_versions", "value" => json_encode($this->rcmail->xversions)]);
            $this->rcmail->xversionsUpdate = true;
        }
     ...
     * @param $html
     */
    private function hideAboutLink(&$html)
    {
