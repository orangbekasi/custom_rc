...
let xframework = new function() {
     ...
     * @returns {string}
     */
    this.getDateFormat = function(type = undefined) {
     ...
     * @returns {string}
     */
    this.getDateTimeFormat = function(type = undefined) {
        return rcmail.env.dateFormats[type === undefined ? "moment" : type] + " " +
     ...
     * @returns {string}
     */
    this.getDmFormat = function(type = undefined) {
...
(function($) {
    $.fn.alterClass = function (removals, additions) {
        if (removals.indexOf('*') === -1) {
            ...
            const patern = new RegExp('\\s' + removals.replace(/\*/g, '[A-Za-z0-9-_]+').split(' ').join('\\s|\\s') + '\\s', 'g');

            this.each(function (i, it) {
                let cn = ' ' + it.className + ' ';

                while (patern.test(cn)) {
                    cn = cn.replace(patern, ' ');
                }
